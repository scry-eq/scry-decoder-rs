...
//!   /*0004*/ z:19,       animation:10,    padding:3
//!   /*0008*/ y:19,       heading:12,      padding:1
//!   /*0012*/ deltaY:13,  pitch:12,        padding:7
//!   /*0016*/ x:19,       deltaX:13
//!   /*0020*/ deltaHeading:10, deltaZ:13,  padding:9
//!   /*0024*/
//! ```
//!
//! Layout re-synced to legacy 6.4.26 (08/19 patch) 2026-08-19. Same 24B size,
//! so no size check catches a mismatch here.
//!
...
pub fn parse_player_spawn_pos(bytes: &[u8]) -> Result<PlayerSpawnPos, PlayerSpawnPosError> {
    ...
    // offset 4: z:19, animation:10, padding:3
    let w0 = read_u32_le(bytes, 4);
    let z = sign_extend(w0 & 0x7_FFFF, 19);
    let animation = sign_extend((w0 >> 19) & 0x3FF, 10) as i16;

    // offset 8: y:19, heading:12, padding:1
    let w1 = read_u32_le(bytes, 8);
    let y = sign_extend(w1 & 0x7_FFFF, 19);
    let heading = ((w1 >> 19) & 0xFFF) as u16;

    // offset 12: deltaY:13, pitch:12, padding:7
    let w2 = read_u32_le(bytes, 12);
    let delta_y = sign_extend(w2 & 0x1FFF, 13);
    let pitch = ((w2 >> 13) & 0xFFF) as u16;

    // offset 16: x:19, deltaX:13
    let w3 = read_u32_le(bytes, 16);
    let x = sign_extend(w3 & 0x7_FFFF, 19);
    let delta_x = sign_extend((w3 >> 19) & 0x1FFF, 13);

    // offset 20: deltaHeading:10, deltaZ:13, padding:9
    let w4 = read_u32_le(bytes, 20);
    let delta_heading = sign_extend(w4 & 0x3FF, 10) as i16;
    let delta_z = sign_extend((w4 >> 10) & 0x1FFF, 13);
...
mod tests {
    ...
    #[test]
    fn z_animation_pack_offset4() {
        let mut buf = [0u8; PAYLOAD_LEN];
        // z=-1 (0x7_FFFF, 19-bit), animation=-1 (0x3FF, 10-bit).
        let w: u32 = 0x7_FFFF | (0x3FFu32 << 19);
    ...
    #[test]
    fn y_heading_pack_offset8() {
        let mut buf = [0u8; PAYLOAD_LEN];
        // y=42 (19-bit), heading=0xABC (12-bit).
        let w: u32 = 42 | (0xABCu32 << 19);
    ...
    #[test]
    fn deltay_pitch_pack_offset12() {
        let mut buf = [0u8; PAYLOAD_LEN];
        // deltaY=7 (13-bit), pitch=0xFFF (12-bit).
        let w: u32 = 7 | (0xFFFu32 << 13);
    ...
    #[test]
    fn x_deltax_pack_offset16() {
        let mut buf = [0u8; PAYLOAD_LEN];
        // x=-1 (0x7_FFFF, 19-bit), deltaX=-2 (0x1FFE, 13-bit).
        let w: u32 = 0x7_FFFF | (0x1FFEu32 << 19);
    ...
    #[test]
    fn deltaheading_deltaz_pack_offset20() {
        let mut buf = [0u8; PAYLOAD_LEN];
        // deltaHeading=-1 (0x3FF, 10-bit), deltaZ=7 (13-bit).
        let w: u32 = 0x3FF | (7u32 << 10);
