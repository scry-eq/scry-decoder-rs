//! Rust mirrors of EQ wire structs, generated by `tools/gen_eqstructs.py test`
//! against `showeq-daemon/src/backend/test/everquest.h`. Identical to
//! `seq-eqstructs-live` today; diverges when the Test server's wire format
//! forks. To add a struct: append it to `ALLOWLIST` in the script, re-run,
//! and commit the regenerated `bindings.rs`.
//!
